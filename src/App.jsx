...
{/* MODAL DE AÇÃO DAS PRÁTICAS (REALIZAR OU JÁ REALIZADO) */}
      ...
      {/* O BALÃO DE ARJUNA (TERMÔMETRO DE CONSCIÊNCIA) */}
      {user && view === 'today' && (
        <>
          {/* Widget Flutuante (O Balão) */}
          <div 
            onClick={() => setShowConsciousnessModal(true)}
            title="Elevação de Consciência"
            style={{ 
              position: 'fixed', left: '20px', 
              bottom: `calc(10% + ${altitude * 0.7}%)`, // O BALÃO SOBE E DESCE SOZINHO!
              zIndex: 9998, cursor: 'pointer', transition: 'bottom 1.5s cubic-bezier(0.25, 1, 0.5, 1)',
          ...
          >
            ...
            {/* Indicador percentual muito discreto */}
            <div style={{ background: isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.8)', padding: '2px 6px', borderRadius: '10px', fontSize: '0.65rem', fontWeight: 'bold', color: isDark ? '#f0e6d2' : '#2c1810', marginTop: '4px', backdropFilter: 'blur(2px)' }}>
              {altitude}%
            </div>
          </div>

          {/* O MODAL DE DIÁLOGO E STATUS */}
          {showConsciousnessModal && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', backdropFilter: 'blur(5px)' }} onClick={() => setShowConsciousnessModal(false)}>
              <div className="animate-fadeIn" style={{ background: isDark ? '#1a1a2e' : '#fdfbf7', padding: '0', borderRadius: '16px', maxWidth: '450px', width: '100%', border: `2px solid ${altitude >= 70 ? '#FFD700' : (altitude <= 30 ? '#e74c3c' : '#8b7355')}`, overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }} onClick={(e) => e.stopPropagation()}>
                
                {/* CABEÇALHO DO MODAL (Muda de cor conforme a altitude) */}
                <div style={{ background: altitude >= 70 ? 'linear-gradient(135deg, rgba(255,215,0,0.2), rgba(255,165,0,0.2))' : (altitude <= 30 ? 'linear-gradient(135deg, rgba(231,76,60,0.2), rgba(192,57,43,0.2))' : 'linear-gradient(135deg, rgba(139,115,85,0.1), rgba(107,68,35,0.1))'), padding: '2rem 1.5rem', textAlign: 'center', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, position: 'relative' }}>
                  <button onClick={() => setShowConsciousnessModal(false)} style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'transparent', border: 'none', color: isDark ? '#f0e6d2' : '#2c1810', cursor: 'pointer' }}><X size={20} /></button>
                  <h2 style={{ margin: '0 0 0.5rem 0', fontFamily: "'Cinzel', serif", color: isDark ? '#f0e6d2' : '#2c1810', fontSize: '1.6rem' }}>Elevação de Consciência</h2>
                  <div style={{ fontSize: '3rem', fontWeight: 'bold', fontFamily: "'Cinzel', serif", color: altitude >= 70 ? (isDark ? '#FFD700' : '#d4af37') : (altitude <= 30 ? '#e74c3c' : (isDark ? '#b8a88a' : '#8b7355')) }}>
                    {altitude}%
                  </div>
                </div>
                ...
                <div style={{ padding: '2rem 1.5rem' }}>
                  ...
                  {altitude <= 30 ? (
                    <div style={{ background: isDark ? 'rgba(231, 76, 60, 0.1)' : '#fff5f5', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid #e74c3c', marginBottom: '1.5rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#e74c3c', textTransform: 'uppercase', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.5rem' }}><Moon size={14} /> Os Kuravas murmuram:</span>
                      <p style={{ margin: 0, fontStyle: 'italic', fontSize: '1.05rem', color: isDark ? '#f0e6d2' : '#2c1810', lineHeight: '1.6' }}>"Sim... continue reagindo aos seus instintos e ignorando suas práticas. A gravidade é o nosso domínio. Deixe a mente afundar na matéria."</p>
                    </div>
                  ) : altitude >= 70 ? (
                    <div style={{ background: isDark ? 'rgba(255, 215, 0, 0.05)' : '#fffbf0', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid #FFD700', marginBottom: '1.5rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: isDark ? '#FFD700' : '#996515', textTransform: 'uppercase', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.5rem' }}><Sun size={14} /> Krishna orienta:</span>
                      <p style={{ margin: 0, fontStyle: 'italic', fontSize: '1.05rem', color: isDark ? '#f0e6d2' : '#2c1810', lineHeight: '1.6' }}>"A sua mente repousa no alto, Arjuna. Firme como uma chama num local sem vento. Mantenha a vigília, pois os ventos da distração sopram fortes."</p>
                    </div>
                  ) : (
                    <div style={{ background: isDark ? 'rgba(255, 255, 255, 0.05)' : '#f5f5f5', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid #8b7355', marginBottom: '1.5rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#8b7355', textTransform: 'uppercase', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.5rem' }}><Mountain size={14} /> O Campo de Batalha:</span>
                      <p style={{ margin: 0, fontStyle: 'italic', fontSize: '1.05rem', color: isDark ? '#f0e6d2' : '#2c1810', lineHeight: '1.6' }}>"A batalha está empatada. Você não caiu nas sombras, mas ainda não se ergueu à luz. Acenda o fogo da vontade com uma ação prática hoje."</p>
                    </div>
                  )}

                  {/* EXPLICAÇÃO TÉCNICA */}
                  <h4 style={{ margin: '0 0 0.5rem 0', color: isDark ? '#b8a88a' : '#6b5744', fontSize: '0.9rem', textTransform: 'uppercase' }}>O que está afetando seu balão:</h4>
                  <ul style={{ margin: 0, paddingLeft: '1.2rem', color: isDark ? '#f0e6d2' : '#2c1810', fontSize: '0.95rem', lineHeight: '1.6' }}>
                    {streak > 0 && <li>A constância dos seus dias (<span style={{ color: '#4caf50' }}>+{Math.min(streak * 2, 20)}%</span>)</li>}
                    {morningDone && <li>A realização do Prólogo Matinal (<span style={{ color: '#4caf50' }}>+10%</span>)</li>}
